...
describe("Auth", () => {
  ...
  it("reinicia el contador de intentos después de un login exitoso", async () => {
    await request(app)
      .post("/api/auth/register")
      .send({ email: "login-reset@test.com", password: "Password1" });

    for (let i = 0; i < 3; i++) {
      const res = await request(app)
        .post("/api/auth/login")
        .send({ email: "login-reset@test.com", password: "Incorrecta1" });

      expect(res.status).toBe(401);
    }

    const loginCorrecto = await request(app)
      .post("/api/auth/login")
      .send({ email: "login-reset@test.com", password: "Password1" });

    expect(loginCorrecto.status).toBe(200);
    ...
    const loginFinal = await request(app)
      .post("/api/auth/login")
      .send({ email: "login-reset@test.com", password: "Password1" });
